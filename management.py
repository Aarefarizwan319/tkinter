...
class HotelRestaurantManager:
        ...
        def game_loop():
            while self.running:
                ...
                if current_time - self.last_update >= 5:
                    ...
                    # Random hotel guest
                    if random.random() < 0.2:  # 20% chance every 5 seconds
                        self.generate_hotel_guest()
                    
                    # Decrease cleanliness over time
                    if random.random() < 0.4:  # 40% chance every 5 seconds
    ...
    def generate_customer(self):
        """Generate a random customer for the restaurant"""
        customer_names = ["Alice", "Bob", "Charlie", "Diana", "Eve", "Frank", "Grace", "Henry"]
        customer_name = f"{random.choice(customer_names)} {random.randint(1, 999)}"
        
        # Random order based on reputation
        items = list(self.menu.keys())
        item = random.choice(items)
        
        # Higher reputation = more likely to get customers
        if random.randint(1, 100) <= self.reputation:
            order_time = time.time()
            order = {
                'customer': customer_name,
                'item': item,
            ...
            # Add to cooking queue
            cooking_time = self.menu[item]['time']
            self.root.after(0, lambda: self.cooking_listbox.insert(tk.END, f"{item} for {customer_name} - {cooking_time}s"))
    ...
    def generate_hotel_guest(self):
        """Generate a random hotel guest"""
        guest_names = ["Mr. Smith", "Ms. Johnson", "Dr. Brown", "Mrs. Davis", "Mr. Wilson", "Ms. Garcia"]
        guest_name = f"{random.choice(guest_names)} {random.randint(1, 999)}"
        
        # Choose room type based on availability and reputation
        available_rooms = []
        for room_type, details in self.rooms.items():
            if details['occupied'] < details['total']:
                available_rooms.append(room_type)
        
        if available_rooms and random.randint(1, 100) <= self.reputation:
            ...
            # Update displays
            self.root.after(0, self.update_bookings_display)
            self.root.after(0, self.update_room_labels)
